...
- Selective prediction: A2 PE entity score floor reduces Cubicle emitted answers `104 -> 91`, retaining 28 strict/78 padded correct answers. No fitted probability calibration. Score-gap abstention is retained as a failed ablation.
- Ceiling audit: GT-assisted selection over the existing Cubicle A2 PE centers can reach only `28/149`, which current selection attains. All existing robust Top-5 observation centers also cap at 28; this is a fixed-center oracle, not a model/geometry limit. Top-1 PCA oracle is 21 vs actual 10, so the chosen common adapter is not an optimized upper bound on the baseline. Apartment union/fallback oracle is 3 vs current 1; it is not an achieved proposed result.
- A staged-files-only temporary checkout also passed all 92 tests and the relation, final-Clio, and PE summary validators. Instance audit found no missing required assets or upstream pin mismatches; available disk was 65.81 GiB and RTX 4090 was detected.
- Validation before publication: 92 CPU tests pass; 20,610 predictions and their scoring replay from local caches; portable summary integrity/count checks pass; original Clio/PE summaries and both frozen relation bundles pass. No new GPU forward pass is claimed.
- Publication scope: current code, diagnostic scripts, text/JSON evidence and documents, plus restored continuity skill/memory/baseline. Input resume PDF and raw datasets/masks/point clouds/checkpoints/full run trees remain outside the publication.
- Partial/pending: robust new-scene candidate localization, independently split confidence calibration, genuine same-category left/right disambiguation, full end-to-end latency/VRAM and generalization. Relation improvements do not provide navigation/control or geometric SLAM optimization.
- Concrete next work: prepare the Friday interview from `docs/RELATION_MODULES.md`; future accuracy work should diagnose the missing/mislocalized candidates before changing rejection thresholds. No additional training, data acquisition or publication is implied.
- Recovery: historical full research tree remains at `research-final-v1-2026-09-02` / `research-final-2026-09-02`, commit `53db410`. This publication restores continuity files that were excluded from the focused `3d312db` tree.
...
| Relation post-D21, 2026-09-10 | Complete at post-hoc controlled-experiment scope | 30 configurations per suite, two scenes, main and coarse-category suites, negative results, source replay, fixed-center oracle and module document. Independent calibration/generalization remain pending. |
...
New local-only dependencies for relation source replay:

- `runs/clio-relation-improvement-v2`: all candidates, 30-method predictions/evaluations, authored diagnostic queries and full validation.
- `runs/clio-pe-semantic-fusion-v1`: saved PE crop scores used for per-object representative-center selection.
- `runs/interview-relation-audit-20260909` and `runs/relation-improvement-feasibility-20260909`: historical development diagnostics; optional.
- A clean clone receives `evidence/relation-improvement` and can validate its summary without these caches; it cannot recreate raw Clio predictions without the local geometry, SAM, PE and task inputs.
...
1. Practise the project geometry, A2 association, relation selection and failure explanations in `docs/RELATION_MODULES.md` for the Friday internship interview.
2. For further experiments, separate candidate availability from point accuracy on the ten remaining Cubicle classes. Existing Top-5 observation centers cannot exceed the current 28 strict pairs. New observations or recomputed clean object geometry are required to raise that fixed-center ceiling.
3. Only after candidate changes, evaluate embedding-assisted matching and selective prediction on an independently held-out scene/object split. Preserve the frozen original queries and report candidate recall, strict/padded pair accuracy, coverage and negative false answers.
