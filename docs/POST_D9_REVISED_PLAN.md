...
> **归档说明（2026-09-02）**：这是阶段路线和历史状态日志，不是当前验收页；其中的 D18 `CLIO_HELD_OUT_PENDING` 仅描述当时快照。当前结果以 README 和 `evidence/final-clio/` 为准。
...
| A2 | `evidence-aware-association` | 通用实现可使用语义一致性、OBB/中心几何、观测质量、跨帧支持和拒绝原因；每次 merge 保存可重算证据 | D12 起的新方法，独立于 Q0/Q1/Q2 比较；最终 Clio 输入无 semantic embedding，实际仅称任务内几何＋质量 complete-link |
