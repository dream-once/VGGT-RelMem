...
from .geometry import save_geometry_npz
...
def validate_upstream_layout(path: str | Path) -> Path:
    ...
    required = (root / "main.py", root / "vggt_slam/solver.py", root / "vggt_slam/submap.py")
    missing = [str(item) for item in required if not item.exists()]
    if missing:
        raise FileNotFoundError(f"invalid VGGT-SLAM checkout; missing: {missing}")
    return root
...
def _frame_id(image_path: str, submap_id: int, index: int, used: set[str]) -> str:
    stem = Path(image_path).stem or f"frame_{submap_id}_{index}"
    candidate = stem if stem not in used else f"{stem}__s{submap_id}_i{index}"
    serial = 2
...
def export_solver_geometry(
    ...
    *,
    ...
    optimized SL(4) global transform and may be projective. Confidence becomes a
    binary valid map using upstream's percentile threshold. Rigid anchor poses
    are written separately.
    """
    output = Path(geometry_path)
    manifest = Path(manifest_path) if manifest_path else output.with_suffix(".manifest.json")
    poses_path = Path(anchor_poses_path) if anchor_poses_path else output.with_suffix(".anchor_poses.json")
    ...
    for submap in submaps:
        ...
        if local_points.ndim != 4 or local_points.shape[-1] != 3:
            raise ValueError(f"submap {submap_id} pointcloud shape is invalid")
        if raw_conf.shape != local_points.shape[:-1]:
            raise ValueError(f"submap {submap_id} confidence shape is invalid")
        names = list(getattr(submap, "img_names", []))
        if len(names) != len(local_points):
            names = [f"submap_{submap_id}_frame_{i}" for i in range(len(local_points))]
        rigid_poses = _array(submap.get_all_poses_world(solver.graph))
        if rigid_poses.shape != (len(local_points), 4, 4):
            raise ValueError(f"submap {submap_id} anchor pose shape is invalid")
        ...
        for index, name in enumerate(names):
            ...
            frame_id = _frame_id(str(name), submap_id, index, used_ids)
            node_id = submap_id + index
            transform = _array(solver.graph.get_homography(node_id)).astype(np.float64)
            if transform.shape != (4, 4) or not np.all(np.isfinite(transform)):
                raise ValueError(f"graph node {node_id} homography is invalid")
            if abs(transform[-1, -1]) > 1e-12:
                transform /= transform[-1, -1]
            points.append(local_points[index].astype(np.float32, copy=False))
            confidence.append((raw_conf[index] > threshold).astype(np.float32))
            transforms.append(transform)
            anchor_poses[frame_id] = rigid_poses[index].astype(float).tolist()
            records.append(VGGTFrameRecord(frame_id, str(name), submap_id, index, node_id, threshold))

    if not points:
        raise ValueError("solver contains no exportable frames")
    if len({item.shape for item in points}) != 1:
        raise ValueError("all exported point maps must share one resolution")
    ...
    manifest.write_text(json.dumps({
        ...
        "point_coordinates": "VGGT submap canonical coordinates",
        "point_transform": "optimized SL(4) global_from_submap; homogeneous divide required",
        "confidence_encoding": "binary mask from upstream percentile threshold",
        "frames": [asdict(item) for item in records],
    }, ensure_ascii=False, indent=2) + "\n")
    poses_path.parent.mkdir(parents=True, exist_ok=True)
    poses_path.write_text(json.dumps(anchor_poses, ensure_ascii=False, indent=2) + "\n")
    size = point_stack.nbytes + confidence_stack.nbytes + transform_stack.nbytes
    return VGGTExportSummary(
        str(output), str(manifest), str(poses_path), len(records), submap_count,
        skipped_lc, skipped_duplicates, tuple(point_stack.shape[1:]), size,
    )
